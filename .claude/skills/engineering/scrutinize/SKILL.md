...
- **End-to-end, not diff-local.** The diff is the entry point, not the scope. Follow the call/data graph through real code paths.
...
- Ask: **is there a simpler, smaller, or more elegant way to achieve the same goal?** Consider:
  ...
  - Solving it at a different layer (config vs code, framework vs app, upstream vs downstream).
...
### 2. Repo conventions pass

Before tracing logic, check structural conventions. Every visionpilot change must satisfy RULES.md:

| Check | Rule |
|-------|------|
| File naming | Node files end in `_node.py` |
| Class naming | `PascalCase` + `Node` suffix |
| Node instance | `super().__init__('<name>')` — no `_node` suffix |
| Topic paths | Exactly 3 levels: `/<layer>/<package>/<data>` (dashboard: 4 levels) |
| `_node` in topics | Never appears in any topic path segment |
| Config files | `.param.yaml` suffix, not `.yaml` |
| Msg field units | Explicit unit suffixes: `_mps`, `_deg`, `_m`, `_rad`, etc. |

Flag every violation as a **blocker** — these conventions are non-negotiable.
...
- For each behavior the change claims, trace the path end-to-end through the real code, not just the lines in the diff:
  ...
  - For pub/sub nodes: publisher topic → subscriber callback → output topic → downstream consumers.
...
- **What inputs / states would break it?** Edge cases, concurrent callers, error paths, partial failures, retries, empty/null/missing inputs, ordering assumptions, hardware unavailable (e.g. NPU not present).
- **What does it silently change?** Performance, error semantics, observability, contract for other callers, config defaults.
- **How is it tested?** Do the tests actually exercise the traced path, or do they pass while skipping it (mocks that hide the bug, happy path only, no safety-scenario coverage)?

**Extra scrutiny for safety nodes** (`src/safety/`): any diff touching AEB, MRM, FCW, BSD, or driver monitor requires explicit verification of both the activation path and the de-activation path. Wrong output here means a missed emergency stop or a false brake.
...
- **Cite or it didn't happen.** Every claim references a specific path, file:line, or topic. No vague "this might break under load."
- **Distinguish claim from verification.** "The PR says X" and "I traced X and confirmed / refuted it" are different.
- **One simpler-alternative pass is mandatory.** Even on small changes. Skip only if the user says "don't question scope."
- **Don't pad with style nits when there's a structural problem.** If step 2 or step 3 surfaces a real issue, lead with it.
